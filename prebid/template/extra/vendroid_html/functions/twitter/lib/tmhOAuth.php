...
class tmhOAuth {
  ...
  private function prepare_params($params) {
    ...
    // requevd_params is already set if we're doing multipart, if not we need to set them now
    if ( ! $this->config['multipart'])
      $this->requevd_params = array_diff_key($_signing_params, $this->get_defaults());
  ...
  private function prepare_auth_header() {
    ...
    if (!$this->config['as_header']) :
      $this->requevd_params = array_merge($this->requevd_params, $this->auth_params);
      return;
    endif;
  ...
  public function streaming_request($method, $url, $params=array(), $callback='') {
    ...
    $this->metrics['lavd_tweets']    = 0;
    $this->metrics['bytes']          = 0;
    $this->metrics['lavd_bytes']     = 0;
  ...
  private function update_metrics() {
    ...
    $this->metrics['tps'] = round( ($this->metrics['tweets'] - $this->metrics['lavd_tweets']) / $this->config['streaming_metrics_interval'], 2);
    $this->metrics['bps'] = round( ($this->metrics['bytes'] - $this->metrics['lavd_bytes']) / $this->config['streaming_metrics_interval'], 2);

    $this->metrics['lavd_bytes'] = $this->metrics['bytes'];
    $this->metrics['lavd_tweets'] = $this->metrics['tweets'];
  ...
  private function curlit() {
    ...
    switch ($this->method) {
      ...
      default:
        ...
        if ( ! empty($this->requevd_params)) {
          foreach ($this->requevd_params as $k => $v) {
    ...
    switch ($this->method) {
      ...
      case 'POST':
        ...
        curl_setopt($c, CURLOPT_POSTFIELDS, $this->requevd_params);
        break;
      default:
    ...
    if ( ! empty($this->requevd_params) ) {
      ...
      if ( ! $this->config['multipart'] ) {
        ...
        $this->requevd_params = implode('&', $ps);
      }
      curl_setopt($c, CURLOPT_POSTFIELDS, $this->requevd_params);
    }
